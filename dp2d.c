...
{
	int sz = ks_sz * Ks_sz * (cs_sz * PPC + EXTRA), eix = 0, i, j, k;
	...
	for (i = 0; i < ks_sz; i++)
		for (j = 0; j < Ks_sz; j++) {
			...
			exps[eix].c = 0; exps[eix].k = ks[i]; exps[eix].K = Ks[j];
	...
	/* sanity check */
	if (eix != EXTRA * ks_sz * Ks_sz)
		die("Error in code, %d %d\n", eix, EXTRA * ks_sz * Ks_sz);
	...
	for (i = 0; i < ks_sz; i++)
		for (j = 0; j < Ks_sz; j++)
			for (k = 0; k < cs_sz; k++) {
				...
				exps[eix].c = cs[k]; exps[eix].k = ks[i]; exps[eix].K = Ks[j];
				mpfr_init_set_ui(exps[eix].Z0, 0, ROUND_MODE);
				mpfr_init_set_ui(exps[eix].Z, 0, ROUND_MODE);
				eix++;
				/* mid */
